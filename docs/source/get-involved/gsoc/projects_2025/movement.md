...
<<<<<<< HEAD:docs/source/get-involved/gsoc/projects_2025.md
:::{grid-item-card} {fas}`video;sd-text-primary` Support for any-point trackers in `ethology`

<!-- intro to ethology -->
[`ethology`](https://github.com/neuroinformatics-unit/ethology) is a Python package in early-development stage, conceived as a sister package to [`movement`](https://movement.neuroinformatics.dev/index.html). While `movement` focuses on the analysis of animal pose estimation data, `ethology` aims to facilitate the application of a wide range of computer vision tasks to animal behaviour research. The goal of `ethology` is to cover tasks that are common in the field, like pose estimation, but also other tasks that may be less common or standard, such as object detection and tracking, segmentation or 3D pose estimation. We would like to lower the barrier to entry for researchers who want to apply these methods to their data, by providing a unified data analysis interface across these tasks. Going forwards, we would like to provide access to the best performing and most robust models, and allow straightforward comparisons across methods.
...
::::
=======
>>>>>>> fa79897 (Add projects "home" page. Add projects section in main GSoC page. Split projects per repo. Make project descriptions collapsible.):docs/source/get-involved/gsoc/projects_2025/movement.md
